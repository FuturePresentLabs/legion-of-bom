...
pub fn placement_design_rules(intent: &PlacementIntent) -> String {
    ...
    for (index, region) in keepouts.into_iter().enumerate() {
        let area = format!("LOB_KEEP_OUT_{index:03}_{}", rule_identifier(&region.name));
        out.push_str(&format!(
            "(rule \"keepout {}\"\n\
...
fn rule_identifier(name: &str) -> String {
    let id = name
        .chars()
        .map(|character| {
...
mod tests {
    ...
    fn placement_rules_compile_keepouts_and_preserve_lob_only_relations() {
        ...
        assert!(dru.contains("A.intersectsArea('LOB_KEEP_OUT_000_RF_antenna___no_copper')"));
